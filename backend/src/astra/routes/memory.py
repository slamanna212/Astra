...
def _config_allows(paths, which: str) -> bool:
    try:
        cfg = yaml.safe_load(paths.config_yaml.read_text(encoding="utf-8")) or {}
    except (OSError, yaml.YAMLError):
        cfg = {}
    memory = cfg.get("memory") if isinstance(cfg, dict) else {}
    return not (
        (which == "memory" and isinstance(memory, dict) and memory.get("memory_enabled") is False)
        or (which == "user" and isinstance(memory, dict) and memory.get("user_profile_enabled") is False)
    )
...
async def files(ctx: Ctx):
    try:
        return MemoryFilesResponse(files=await anyio.to_thread.run_sync(_read, ctx.settings.paths))
    except PermissionError:
        raise HTTPException(403, "Refusing a symlinked memory file") from None


@router.put("/files/{which}", status_code=204)
async def write(which: str, body: MemoryWriteRequest, ctx: Ctx):
    try:
        await anyio.to_thread.run_sync(_write, ctx.settings.paths, which, body.content)
    except ValueError:
        raise HTTPException(404, "Memory file not found") from None
    except PermissionError:
        raise HTTPException(403, "Memory file is disabled or symlinked") from None
